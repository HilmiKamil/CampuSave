...
.transaction-list::-webkit-scrollbar-thumb:hover {
  ...
  <main class="container py-5">
    ...
    <!-- Card Utama Saldo -->
<div class="p-4 bg-green-custom rounded text-white shadow-lg w-100" style="max-width: 100%; border-radius: 16px;">
            ...
            <div class="d-flex justify-content-between align-items-center">
              <div>
                  <h6 class="mb-1 fw-semibold">
                      {{ $transaksi->budgetCategory->name ?? $transaksi->category }}
                  </h6>
              ...
              <div class="{{ $transaksi->type === 'pengeluaran' ? 'text-danger' : 'text-success' }}">
                <span class="fw-bold">
                  {{ $transaksi->type === 'pengeluaran' ? '-' : '+' }}Rp {{ number_format($transaksi->amount, 0, ',', '.') }}
                </span>
              </div>
            </div>
    ...
    <div class="modal-dialog">
      <form method="POST" action="{{ route('user.transaksi.store') }}">
        ...
        @endif
            ...
            <select name="budget_category_id" class="form-select" id="income-category" required>
                <option value="">Pilih Kategori</option>
                @foreach($categories as $category)
                    <option value="{{ $category->id }}">{{ $category->name }}</option>
                @endforeach
